...
# -> "prob_norm": parameter that determines if treatment probabilities are 
# normalized to sum to one within an observation.
...
func_ml_lasso <- function(treatment_setting, data_train, data_test, outcome, 
                          ...
                          post = FALSE, prob_norm = "yes", probscore_separate = TRUE) {
  ...
  #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#
    ...
    if (probscore_separate == FALSE) {
      ...
      K_folds_inner_m <- rsample::group_vfold_cv(
        data = data_train %>%
          group_by(group) %>% 
          mutate("treatment_sport_freq" = as.numeric(as.character(treatment_sport_freq))) %>%
          mutate(treatment_fold = mean(!!rlang::sym(treatment))) %>%
          mutate("treatment_sport_freq" = as.factor(treatment_sport_freq)) %>% 
          ungroup(), 
        v = K, group = group, strata = treatment_fold, balance = "observations"
      )
      K_folds_inner_g1 <- rsample::group_vfold_cv(
      ...
      lasso_grid_search_g2 <- 
        lasso_workflow_g2 %>%
        tune_grid(resamples = K_folds_inner_g2, grid = lasso_grid, 
                  metrics = metric_set(rmse))
      lasso_grid_search_g3 <- 
        lasso_workflow_g3 %>%
        tune_grid(resamples = K_folds_inner_g3, grid = lasso_grid, 
                  metrics = metric_set(rmse))
      ...
      lasso_best_param_g2 <- lasso_best_param_g2$penalty
      
      lasso_best_param_g3 <- lasso_grid_search_g3 %>% select_best("rmse")
      lasso_best_param_g3 <- lasso_best_param_g3$penalty
        
      df_best_param <- data.frame(
        "m" = lasso_best_param_m, "g1" = lasso_best_param_g1, "g2" = lasso_best_param_g2, "g3" = lasso_best_param_g3
        )
      
      # specify the models
      lasso_best_param_m <- df_best_param$m
      lasso_spec_final_m <- 
        multinom_reg(penalty = {{lasso_best_param_m}}, mixture = 1) %>%  
        set_engine("glmnet") %>%  
        set_mode("classification") 
      ...
      lasso_coef_all <- lasso_coef_m %>%
        mutate(model = case_when(
          class == "1" ~ "m1", class == "2" ~ "m2", class == "3" ~ "m3", TRUE ~ "NA"
        )) %>% dplyr::select(-class)
      lasso_coef_all <- rbind(lasso_coef_all, lasso_coef_g1)
      ...
      # data set with number of coefficients
      lasso_coef_num <- data.frame("m" = nrow(lasso_coef_m), "g1" = nrow(lasso_coef_g1),
                                   "g2" = nrow(lasso_coef_g2), "g3" = nrow(lasso_coef_g3))
      ...
      if (post == TRUE) {
        ...
        data_train_final <- data_train %>% dplyr::select(all_of(outcome), all_of(treatment), all_of(lasso_coef_union))
        data_train_final_m <- data_train_final %>% dplyr::select(-all_of(outcome))
        data_train_final_g1 <- data_train_final %>% filter(treatment_sport_freq == 1) %>% dplyr::select(-all_of(treatment))
        data_train_final_g2 <- data_train_final %>% filter(treatment_sport_freq == 2) %>% dplyr::select(-all_of(treatment))
        data_train_final_g3 <- data_train_final %>% filter(treatment_sport_freq == 3) %>% dplyr::select(-all_of(treatment))
        ...
        vars_multicoll_drop_1 <- c(
          attributes(alias(model_m)$Complete)$dimnames[[1]],
          attributes(alias(model_lm_1)$Complete)$dimnames[[1]],
          attributes(alias(model_lm_2)$Complete)$dimnames[[1]], 
        ...
        vars_multicoll_drop_2 <- c(
          names(VIF(model_m)[VIF(model_m) > 5]), names(VIF(model_lm_2)[VIF(model_lm_2) > 5]), 
          names(VIF(model_lm_1)[VIF(model_lm_1) > 5]), names(VIF(model_lm_3)[VIF(model_lm_3) > 5])) %>% unique()
        lasso_coef_all <- lasso_coef_all %>% filter(!term %in% vars_multicoll_drop_2)
        lasso_coef_union <- colnames(data_train_final)[colnames(data_train_final) %in% unique(lasso_coef_all$term)]
        data_train_final <- data_train_final %>% dplyr::select(all_of(outcome), all_of(treatment), all_of(lasso_coef_union))
        data_train_final_m <- data_train_final %>% dplyr::select(-all_of(outcome))
        ...
        # final
        X_controls_final <- data_train_final %>% 
          dplyr::select(-c(all_of(outcome), starts_with(treatment))) %>% colnames()
        
        lasso_recipe_m <- 
          data_train_final_m %>%
          recipe(.) %>%
          update_role({{treatment}}, new_role = "outcome") %>%
          update_role(all_of(X_controls_final), new_role = "predictor")
        ...
        lasso_fit_final_m <- 
          lasso_workflow_final_m %>%
          fit(data_train_final_m)
        
        lasso_fit_final_g1 <- 
          lasso_workflow_final_g1 %>%
          fit(data_train_final_g1)
        
        lasso_fit_final_g2 <- 
          lasso_workflow_final_g2 %>%
          fit(data_train_final_g2)
        
        lasso_fit_final_g3 <- 
          lasso_workflow_final_g3 %>%
          fit(data_train_final_g3)
        
        
        # make predictions on test data
        lasso_pred_m <- predict(lasso_fit_final_m, data_test_final_m, type = "prob")
        
        lasso_pred_g1 <- predict(lasso_fit_final_g1, data_test_final_g)
        lasso_pred_g1 <- lasso_pred_g1$.pred
        
        lasso_pred_g2 <- predict(lasso_fit_final_g2, data_test_final_g)
        lasso_pred_g2 <- lasso_pred_g2$.pred
        
        lasso_pred_g3 <- predict(lasso_fit_final_g3, data_test_final_g)
        lasso_pred_g3 <- lasso_pred_g3$.pred
        
        # create prediction data frame
        df_pred <- data.frame(
          # predictions
          "m1" = lasso_pred_m$.pred_1, "m2" = lasso_pred_m$.pred_2, "m3" = lasso_pred_m$.pred_3,
          "g1" = lasso_pred_g1, "g2" = lasso_pred_g1, "g3" = lasso_pred_g1,
          # true values
          "treatment" = data_test %>% pull(treatment), 
          "outcome" = data_test %>% pull(outcome),
          # number of predictors
      ...
      if (hyperparam_sel == "best") {
        ...
        # append results from error metrics
        m1_error_metrics_all <- data.frame()
        for (K_sel in 1:K) {
          m1_error_metrics_all <- rbind(m1_error_metrics_all, lasso_grid_search_m1$.metrics[[K_sel]])
        }
        
        m2_error_metrics_all <- data.frame()
        for (K_sel in 1:K) {
          m2_error_metrics_all <- rbind(m2_error_metrics_all, lasso_grid_search_m2$.metrics[[K_sel]])
        }
        
        m3_error_metrics_all <- data.frame()
        for (K_sel in 1:K) {
          m3_error_metrics_all <- rbind(m3_error_metrics_all, lasso_grid_search_m2$.metrics[[K_sel]])
        }
        
        # aggregate across K
        m1_error_metrics_all <- m1_error_metrics_all %>% group_by(penalty) %>% summarize(
          # average AUC
          AUC = mean(`.estimate`), 
          # standard error: standard deviation divided by the number of folds
          se = sd(`.estimate`) / sqrt(K)
        ) %>% ungroup()
        m1_error_metrics_all <- m1_error_metrics_all %>% arrange(penalty)
        
        m2_error_metrics_all <- m2_error_metrics_all %>% group_by(penalty) %>% summarize(
          AUC = mean(`.estimate`), 
          se = sd(`.estimate`) / sqrt(K)
        ) %>% ungroup()
        m2_error_metrics_all <- m2_error_metrics_all %>% arrange(penalty)
        
        m3_error_metrics_all <- m3_error_metrics_all %>% group_by(penalty) %>% summarize(
          AUC = mean(`.estimate`), 
          se = sd(`.estimate`) / sqrt(K)
        ) %>% ungroup()
        m3_error_metrics_all <- m3_error_metrics_all %>% arrange(penalty)
        
        
        # best: lambda which leads to best error metrics
        lambda_best_m1 <- m1_error_metrics_all %>% filter(AUC == max(AUC)) %>% pull(penalty)
        lambda_best_m2 <- m2_error_metrics_all %>% filter(AUC == max(AUC)) %>% pull(penalty)
        lambda_best_m3 <- m3_error_metrics_all %>% filter(AUC == max(AUC)) %>% pull(penalty)
        
        # calculate standard error of best lambda
        lambda_best_m1_se <- m1_error_metrics_all %>% filter(penalty == lambda_best_m1) %>% pull(se) 
        lambda_best_m2_se <- m2_error_metrics_all %>% filter(penalty == lambda_best_m2) %>% pull(se) 
        lambda_best_m3_se <- m3_error_metrics_all %>% filter(penalty == lambda_best_m3) %>% pull(se) 
        
        # one-standard deviation rule: "largest value of lambda (-> simpler model) such that error is 
        # within 1 standard error of the cross-validated errors for best lambda".
        m1_error_metrics_all_simpler <- m1_error_metrics_all %>% filter(penalty >= lambda_best_m1)
        lambda_se_m1 <- m1_error_metrics_all_simpler %>%
          filter(AUC > max(m1_error_metrics_all %>% pull(AUC)) - lambda_best_m1_se) %>%
          tail(1) %>% pull(penalty) 
        
        m2_error_metrics_all_simpler <- m2_error_metrics_all %>% filter(penalty >= lambda_best_m2)
        lambda_se_m2 <- m2_error_metrics_all_simpler %>%
          filter(AUC > max(m2_error_metrics_all %>% pull(AUC)) - lambda_best_m2_se) %>%
          tail(1) %>% pull(penalty) 
        
        m3_error_metrics_all_simpler <- m3_error_metrics_all %>% filter(penalty >= lambda_best_m3)
        lambda_se_m3 <- m3_error_metrics_all_simpler %>%
          filter(AUC > max(m3_error_metrics_all %>% pull(AUC)) - lambda_best_m3_se) %>%
          tail(1) %>% pull(penalty) 
        ...
        # append results from error metrics
        g2_error_metrics_all <- data.frame()
        for (K_sel in 1:K) {
          g2_error_metrics_all <- rbind(g2_error_metrics_all, lasso_grid_search_g2$.metrics[[K_sel]])
        }
        
        # aggregate across K
        g2_error_metrics_all <- g2_error_metrics_all %>% group_by(penalty) %>% summarize(
          # average AUC
          RMSE = mean(`.estimate`), 
          # standard error: standard deviation divided by the number of folds
          se = sd(`.estimate`) / sqrt(K)
        ) %>% ungroup()
        g2_error_metrics_all <- g2_error_metrics_all %>% arrange(penalty)
        
        # best: lambda which leads to best error metrics
        lambda_best_g2 <- g2_error_metrics_all %>% filter(RMSE == min(RMSE)) %>% pull(penalty)
        
        # calculate standard error of best lambda
        lambda_best_g2_se <- g2_error_metrics_all %>% filter(penalty == lambda_best_g2) %>% pull(se) 
        
        # one-standard deviation rule: "largest value of lambda (-> simpler model) such that error is 
        # within 1 standard error of the cross-validated errors for best lambda".
        g2_error_metrics_all_simpler <- g2_error_metrics_all %>% filter(penalty >= lambda_best_g2)
        lambda_se_g2 <- g2_error_metrics_all_simpler %>%
          filter(RMSE < min(g2_error_metrics_all %>% pull(RMSE)) + lambda_best_g2_se) %>%
          tail(1) %>% pull(penalty) 
        ...
        # append results from error metrics
        g3_error_metrics_all <- data.frame()
        for (K_sel in 1:K) {
          g3_error_metrics_all <- rbind(g3_error_metrics_all, lasso_grid_search_g3$.metrics[[K_sel]])
        }
        
        # aggregate across K
        g3_error_metrics_all <- g3_error_metrics_all %>% group_by(penalty) %>% summarize(
          # average AUC
          RMSE = mean(`.estimate`), 
          # standard error: standard deviation divided by the number of folds
          se = sd(`.estimate`) / sqrt(K)
        ) %>% ungroup()
        g3_error_metrics_all <- g3_error_metrics_all %>% arrange(penalty)
        
        # best: lambda which leads to best error metrics
        lambda_best_g3 <- g3_error_metrics_all %>% filter(RMSE == min(RMSE)) %>% pull(penalty)
        
        # calculate standard error of best lambda
        lambda_best_g3_se <- g3_error_metrics_all %>% filter(penalty == lambda_best_g3) %>% pull(se) 
        
        # one-standard deviation rule: "largest value of lambda (-> simpler model) such that error is 
        # within 1 standard error of the cross-validated errors for best lambda".
        g3_error_metrics_all_simpler <- g3_error_metrics_all %>% filter(penalty >= lambda_best_g3)
        lambda_se_g3 <- g3_error_metrics_all_simpler %>%
          filter(RMSE < min(g3_error_metrics_all %>% pull(RMSE)) + lambda_best_g3_se) %>%
          tail(1) %>% pull(penalty) 
        
        df_best_param <- data.frame(
          "m1" = lambda_se_m1, "m2" = lambda_se_m2, "m3" = lambda_se_m3,
          "g1" = lambda_se_g1, "g2" = lambda_se_g2, "g3" = lambda_se_g3
        )
