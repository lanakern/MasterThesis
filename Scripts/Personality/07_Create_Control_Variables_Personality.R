...
#+++
# 2.) Treatment and outcome: aggregate sport frequency variable,
# generate lagged variables, NA dummies.
#+++
# 3.) Rename categories: To simplify the generation of dummy variables, and let
# the dummy variables have more appealing names, categories are re-named, 
# for instance "[m] male" to "male".
#+++
# 4.) Combine CATI & CAWI Variables: Some variables are measured in both 
# interviews. Here, they are combined.
#+++
# 5.) Replace missing values using MICE
# -> Afterwards all steps are carried out through all m = 5 MICE data sets.
#+++
# 6.) Aggregate variables using cronbach's alpha and pca.
#+++
# 7.) Categorize specific variables.
#+++
...
if (cohort_prep == "controls_same_outcome") {
  load_data <- paste0("Data/Personality/Prep_6/prep_6_sample_selection_", treatment_def, 
                      "_", treatment_repl, extra_act_save, "_personality.rds")
} else {
  load_data <- paste0("Data/Personality/Prep_6/prep_6_sample_selection_", treatment_def, 
                      "_", treatment_repl, extra_act_save, "_personality_robustcheck.rds")
}
...
if (id_num == 0) {
  print("No respondents. Combination is skipped.")
} else {
  ...
  data_prep_1 <- data_prep_1 %>%
    ...
    dplyr::select(-c(health_weight, health_height))
  ...
  data_prep_1 <- data_prep_1 %>%
    ...
    dplyr::select(-health_smoking_v1)
  ...
  data_prep_1 <- data_prep_1 %>%
    ...
    dplyr::select(-c("health_smoking","health_smoking_number"))
  ...
  data_prep_1 <- data_prep_1 %>%
    ...
    # drop v1 variable
    dplyr::select(-c(health_alcohol_v1, health_alcohol))
  ...
  colSums(is.na(data_prep_1 %>% dplyr::select(starts_with("educ_year"))))
  ...
  summary(data_prep_1$educ_years_current_Emp)
  sum(is.na(data_prep_1 %>% dplyr::select(starts_with("spell_length"))))
  ...
  data_prep_1 <- data_prep_1 %>%
    mutate(treatment_period_length = as.numeric(
      difftime(interview_date_end, interview_date_start, units = "days")
    ) / 30)
  ...
  data_prep_1 <- data_prep_1 %>%
    ...
    dplyr::select(-c("educ_school_country", "educ_school_type", "educ_school_quali"))
  ...
  ) %>% dplyr::select(-c(starts_with("mother_country"), starts_with("father_country")))
  ...
  )) %>% dplyr::select(-childhood_parents)
  ...
  data_prep_1 <- data_prep_1 %>%
    ...
    dplyr::select(
      -c(mother_living_ger, starts_with("mother_educ_school_degree"), mother_language_first, 
  ...
  data_prep_1 <- data_prep_1 %>%
    ...
    dplyr::select(
      -c(father_living_ger, starts_with("father_educ_school_degree"), father_language_first, 
  ...
  data_prep_1 <- data_prep_1 %>%
    ...
    mutate(uni_prob_graduation = case_when(
      ...
      TRUE ~ uni_prob_graduation)) %>% 
    dplyr::select(-uni_degree_complete_prob)
  ...
  data_prep_1 <- data_prep_1 %>%
    ...
    dplyr::select(-c(emp_current_student_job, emp_current_student_job_type, emp_current_prof_pos, emp_current_student_job_rel))
  ...
  ## AGGREGATE SPORT FREQUENCY VARIABLE ##
  #++++++++++++++++++++++++++++++++++++++#
  ...
  data_prep_1 <- data_prep_1 %>%
    ...
    mutate(
      ...
      bigfive_extraversion_lag_NA = ifelse(is.na(bigfive_extraversion_lag), 1, 0),
      bigfive_agreeableness_lag_NA = ifelse(is.na(bigfive_agreeableness_lag), 1, 0),
      bigfive_conscientiousness_lag_NA = ifelse(is.na(bigfive_conscientiousness_lag), 1, 0),
      bigfive_neuroticism_lag_NA = ifelse(is.na(bigfive_neuroticism_lag), 1, 0),
      bigfive_openness_lag_NA = ifelse(is.na(bigfive_openness_lag), 1, 0)
    ) #%>%
    # mutate(
    ...
    #   bigfive_agreeableness_lag = case_when(is.na(bigfive_agreeableness_lag) ~ 0, TRUE ~ bigfive_agreeableness_lag),
    #   bigfive_extraversion_lag = case_when(is.na(bigfive_extraversion_lag) ~ 0, TRUE ~ bigfive_extraversion_lag),
    #   bigfive_conscientiousness_lag = case_when(is.na(bigfive_conscientiousness_lag) ~ 0, TRUE ~ bigfive_conscientiousness_lag),
    #   bigfive_neuroticism_lag = case_when(is.na(bigfive_neuroticism_lag) ~ 0, TRUE ~ bigfive_neuroticism_lag),
    #   bigfive_openness_lag = case_when(is.na(bigfive_openness_lag) ~ 0, TRUE ~ bigfive_openness_lag)
    # )
  
  table(data_prep_1$treatment_sport_lag, useNA = "always")
  table(data_prep_1$treatment_sport_freq_lag, useNA = "always")
  table(data_prep_1$bigfive_agreeableness_lag, useNA = "always")
  table(data_prep_1$bigfive_extraversion_lag, useNA = "always")
  table(data_prep_1$bigfive_conscientiousness_lag, useNA = "always")
  table(data_prep_1$bigfive_neuroticism_lag, useNA = "always")
  table(data_prep_1$bigfive_openness_lag, useNA = "always")
  
  
  table(data_prep_1$treatment_sport_lag_NA, useNA = "always")
  table(data_prep_1$treatment_sport_freq_lag_NA, useNA = "always")
  table(data_prep_1$bigfive_agreeableness_lag_NA, useNA = "always")
  table(data_prep_1$bigfive_extraversion_lag_NA, useNA = "always")
  table(data_prep_1$bigfive_conscientiousness_lag_NA, useNA = "always")
  table(data_prep_1$bigfive_neuroticism_lag_NA, useNA = "always")
  table(data_prep_1$bigfive_openness_lag_NA, useNA = "always")
  
  # ensure that no missing values in lagged variables are left
  # colSums(is.na(data_prep_1 %>% ungroup() %>% dplyr::select(
  #   "treatment_sport_lag", "treatment_sport_freq_lag", "bigfive_agreeableness_lag",
  #   "bigfive_extraversion_lag", "bigfive_conscientiousness_lag", "bigfive_neuroticism_lag",
  #   "bigfive_openness_lag"
  # )))
  ...
  sum(data_prep_1$bigfive_agreeableness != data_prep_1$bigfive_agreeableness_lag)
  sum(data_prep_1$bigfive_extraversion != data_prep_1$bigfive_extraversion_lag)
  sum(data_prep_1$bigfive_conscientiousness != data_prep_1$bigfive_conscientiousness_lag)
  sum(data_prep_1$bigfive_neuroticism != data_prep_1$bigfive_neuroticism_lag)
  sum(data_prep_1$bigfive_openness != data_prep_1$bigfive_openness_lag)
  ...
  table(data_prep_1$bigfive_extraversion_NA, useNA = "always")
  table(data_prep_1$bigfive_conscientiousness_NA, useNA = "always")
  table(data_prep_1$bigfive_agreeableness_NA, useNA = "always")
  table(data_prep_1$bigfive_neuroticism_NA, useNA = "always")
  table(data_prep_1$bigfive_openness_NA, useNA = "always")
  ...
  data_prep_1 <- data_prep_1 %>%
    ...
    dplyr::select(-c(educ_highest_degree_casmin, educ_highest_degree_isced, educ_profession_expected))
  ...
  data_prep_1 <- data_prep_1 %>%
    ...
    ) %>% dplyr::select(-c(father_emp_prof_blk, father_emp_prof_egp, father_emp_prof_isei, father_emp_prof_pos,
  ...
  data_prep_1 <- data_prep_1 %>%
    ...
    ) %>% dplyr::select(-c(health_allergic, health_neuro)) 
  ...
  summary(unname((colSums(is.na(data_prep_2 %>% dplyr::select(all_of(col_miss)))) / nrow(data_prep_2))*100))
  ...
  # a student job etc.
  sum(is.na(data_prep_2 %>% dplyr::select(starts_with("emp_current_"))))
  data_prep_2 <- data_prep_2 %>%
    mutate(across(starts_with("emp_current_"), ~ ifelse(emp_current == 0, 0, .)))
  sum(is.na(data_prep_2 %>% dplyr::select(starts_with("emp_current_"))))
  sum(is.na(data_prep_2 %>% filter(emp_current == 0) %>% dplyr::select(starts_with("emp_current_"))))
  
  # Partner (already done in 04_c)
  sum(is.na(data_prep_2 %>% dplyr::select(starts_with("partner_"))))
  sum(is.na(data_prep_2 %>% filter(partner_current == 0) %>% dplyr::select(starts_with("partner_"))))
  
  # Child (already done in 04_c)
  sum(is.na(data_prep_2 %>% dplyr::select(starts_with("child_"))))
  sum(is.na(data_prep_2 %>% filter(child == 0) %>% dplyr::select(starts_with("child_"))))
  
  # Sibling (already done in 04_c)
  sum(is.na(data_prep_2 %>% dplyr::select(starts_with("sibling_"))))
  sum(is.na(data_prep_2 %>% filter(sibling == 0) %>% dplyr::select(starts_with("sibling_"))))
  ...
  col_keep <- c("comp_", "educ_uni_degree_achieve", "educ_uni_degree_aspire", "bigfive",
  ...
  data_prep_2 <- data_prep_2 %>%
    ungroup() %>% 
    dplyr::select(-all_of(col_names_na_drop))
  ...
  vars_drop <- c(
    "educ_uni_start", "birth_date", "day", "educ_study", "educ_highest_degree",
  ...
  pred_matrix[, c(data_prep_4_wide %>% dplyr::select(starts_with("interview_date")) %>% colnames())] <- 0
  ...
  pred_matrix_vars_num <- pred_matrix %>% as.data.frame() %>% 
    dplyr::select(contains("personality")) %>% 
    colnames()
  pred_matrix_vars_set_0 <- pred_matrix %>% as.data.frame() %>% 
    dplyr::select(!c(contains("personality"))) %>% 
    colnames()
  pred_matrix[pred_matrix_vars_num, c(pred_matrix_vars_set_0)] <- 0
  ...
  saveRDS(data_result_mice, 
          paste0("Data/Personality/Prep_7/prep_7_mice_", str_remove_all(cohort_prep, "_"), "_",
                 treatment_def, "_", treatment_repl, extra_act_save, "_personality.rds"))
  gc()
  ...
  for (mice_result_sel in 1:mice_num_data_sets) {
    ...
    data_prep_4_reshape <- data_prep_3 %>% dplyr::select(ID_t, treatment_period)
    ...
    for (col_reshape_sel in col_reshape) {
      ...
      ## select columns
      data_result_mice_sub <- data_mice_sub %>% dplyr::select(ID_t, all_of(col_reshape_sel_all))
    ...
    # show example
    data_prep_5 %>% dplyr::select(ID_t, matches("uni_counsel_.*_offer")) %>% head(5)
    data_prep_5 %>% dplyr::select(ID_t, starts_with("personality_goal_pers")) %>% head(5)
    data_prep_5 %>% dplyr::select(ID_t, starts_with("friends_opinion_degree")) %>% head(5)
    data_prep_5 %>% dplyr::select(ID_t, starts_with("satisfaction_study")) %>% head(5)
    data_prep_5 %>% dplyr::select(ID_t, starts_with("uni_achievement_comp")) %>% head(5)
    ...
    data_prep_5 %>% dplyr::select(ID_t, matches("uni_counsel_offer")) %>% head(5)
    data_prep_5 %>% dplyr::select(ID_t, starts_with("personality_goal_pers"))  %>% head(5)
    data_prep_5 %>% dplyr::select(ID_t, starts_with("friends_opinion_degree"))  %>% head(5)
    data_prep_5 %>% dplyr::select(ID_t, starts_with("satisfaction_study")) %>% head(5)
    data_prep_5 %>% dplyr::select(ID_t, starts_with("uni_achievement_comp")) %>% head(5)
    ...
    data_prep_5 <- data_prep_5 %>% 
      ...
      dplyr::select(-(all_of(cols_offers)))
    ...
    if (aggr_vars == "mean") {
      ...
      list_cawi_labels <- readRDS("Data/Personality/Prep_1/prep_1_target_cawi_list_personality.rds")
      list_cati_labels <- readRDS("Data/Personality/Prep_1/prep_1_target_cati_list_personality.rds")
      ...
      for (vars_relabel in vars_not_aggr_all) {
        ...
        # column names
        colnames_relabel <- data_prep_5 %>% dplyr::select(starts_with(vars_relabel)) %>% colnames()
    ...
    cols_helpless <- data_prep_5 %>% dplyr::select(starts_with("helpless")) %>% colnames()
    ...
    vars_recode_apply <- data_prep_5 %>%
      ...
      dplyr::select_if(~ any(. == "does completely apply")) %>%
      colnames()
    ## recode variables
    data_prep_5 <- data_prep_5 %>% 
      mutate_at(
    ...
    vars_recode_good <- data_prep_5 %>%
      dplyr::select_if(~ is.character(.)) %>%
      dplyr::select_if(~ any(. == "very bad") | any(. == "very high") | any(. == "very important") | any(. == "slightly less")) %>%
      colnames()
    ...
    data_prep_5 %>% dplyr::select(starts_with("uni_quality")) %>% head(5)
    data_prep_5 %>% dplyr::select(starts_with("friends_opinion_degree")) %>% head(5)
    data_prep_5 %>% dplyr::select(starts_with("uni_offers")) %>% head(5)
    data_prep_5 %>% dplyr::select(starts_with("uni_achievement_comp")) %>% head(5)
    ...
    if (data_prep_7 %>% dplyr::select(starts_with("opinion_educ")) %>% colnames() %>% length() > 1 &
        is.character(data_prep_7$opinion_educ_1)) {
    ...
    if (data_prep_7 %>% dplyr::select(starts_with("parents_importance")) %>% colnames() %>% length() > 1 &
        is.character(data_prep_7$parents_importance_success_1)) {
      data_prep_7 <- data_prep_7 %>%
    ...
    if (data_prep_7 %>% dplyr::select(starts_with("parents_opinion_degree")) %>% colnames() %>% length() > 1 &
        is.character(data_prep_7$parents_opinion_degree_1)) {
      data_prep_7 <- data_prep_7 %>%
    ...
    if (data_prep_7 %>% dplyr::select(starts_with("stress")) %>% colnames() %>% length() > 1 &
        is.character(data_prep_7$stress_1)) {
      data_prep_7 <- data_prep_7 %>%
    ...
    if (data_prep_7 %>% dplyr::select(starts_with("uni_offers") & ends_with("helpful")) %>% colnames() %>% length() > 1 &
        is.character(data_prep_7$uni_offers_central_facilities_helpful)) {
      data_prep_7 <- data_prep_7 %>%
    ...
    vars_categoric <- data_prep_7 %>% ungroup() %>% dplyr::select_if(~ is.character(.)) %>% colnames()
    ...
    data_prep_7 <- data_prep_7 %>% rename(birth_month_num = birth_month)
    ...
    ) %>% dplyr::select(-c("birth_month_name", "birth_year", "interview_start_year", "interview_end_year"))
    ...
    if (cohort_prep == "controls_same_outcome") {
      data_save <- paste0("Data/Personality/Prep_7/prep_7_control_vars_", treatment_def, "_", 
                          treatment_repl, extra_act_save, "_mice", mice_result_sel, "_personality.rds")
    } else {
      data_save <- paste0("Data/Personality/Prep_7/prep_7_control_vars_", treatment_def, "_", 
                          treatment_repl, extra_act_save, "_robustcheck", "_mice", 
                          mice_result_sel, "_personality.rds")
    }
...
# if (cohort_prep == "controls_same_outcome") {
#   data_mice_1 <- readRDS(
#     paste0("Data/Personality/Prep_7/prep_7_control_vars_", treatment_def, "_", 
#            treatment_repl, extra_act_save, "_mice", 1, "_personality.rds")
#   )
#   
#   data_mice_2 <- readRDS(
#     paste0("Data/Personality/Prep_7/prep_7_control_vars_", treatment_def, "_", 
#            treatment_repl, extra_act_save, "_mice", 2, "_personality.rds")
#   )
#   
#   data_mice_3 <- readRDS(
#     paste0("Data/Personality/Prep_7/prep_7_control_vars_", treatment_def, "_", 
#            treatment_repl, extra_act_save, "_mice", 3, "_personality.rds")
#   )
#   
#   data_mice_4 <- readRDS(
#     paste0("Data/Personality/Prep_7/prep_7_control_vars_", treatment_def, "_", 
#            treatment_repl, extra_act_save, "_mice", 4, "_personality.rds")
#   )
#   
#   data_mice_5 <- readRDS(
#     paste0("Data/Personality/Prep_7/prep_7_control_vars_", treatment_def, "_", 
#            treatment_repl, extra_act_save, "_mice", 5, "_personality.rds")
#   )
# } else {
#   data_mice_1 <- readRDS(
#     paste0("Data/Personality/Prep_7/prep_7_control_vars_", treatment_def, "_", 
#            treatment_repl, extra_act_save, "_robustcheck", "_mice", 1, "_personality.rds")
#   )
#   
#   data_mice_2 <- readRDS(
#     paste0("Data/Personality/Prep_7/prep_7_control_vars_", treatment_def, "_", 
#            treatment_repl, extra_act_save, "_robustcheck", "_mice", 2, "_personality.rds")
#   )
#   
#   data_mice_3 <- readRDS(
#     paste0("Data/Personality/Prep_7/prep_7_control_vars_", treatment_def, "_", 
#            treatment_repl, extra_act_save, "_robustcheck", "_mice", 3, "_personality.rds")
#   )
#   
#   data_mice_4 <- readRDS(
#     paste0("Data/Personality/Prep_7/prep_7_control_vars_", treatment_def, "_", 
#            treatment_repl, extra_act_save, "_robustcheck", "_mice", 4, "_personality.rds")
#   )
#   
#   data_mice_5 <- readRDS(
#     paste0("Data/Personality/Prep_7/prep_7_control_vars_", treatment_def, "_", 
#            treatment_repl, extra_act_save, "_robustcheck", "_mice", 5, "_personality.rds")
#   )
# }
# 
# 
...
#   if (cohort_prep == "controls_same_outcome") {
#     data_save <- paste0("Data/Personality/Prep_7/prep_7_control_vars_", treatment_def, "_", 
#                         treatment_repl, extra_act_save, "_mice", mice_sel, "_personality.rds")
#   } else {
#     data_save <- paste0("Data/Personality/Prep_7/prep_7_control_vars_", treatment_def, "_", 
#                         treatment_repl, extra_act_save, "_robustcheck", "_mice", 
#                         mice_sel, "_personality.rds")
#   }
#   
