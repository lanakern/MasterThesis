...
# -> "controls_bef_outcome": CATI CAWI CATI
#++++
...
# load data 
data_cohort_profile_raw <- readRDS("Data/Prep_1/prep_1_cohort_profile.rds")
...
data_cohort_profile %>% 
  select(ID_t, competence_month, competence_year) %>%
  group_by(ID_t) %>%
  # count number of NAs for each interview_* column
  summarise_all(~sum(is.na(.))) %>% 
  # take the row sum
...
if (cohort_prep == "controls_bef_outcome") {
  ...
  data_cohort_profile_prep_1 <- data_cohort_profile_prep_1 %>%
    select(-c(wave_2_lag, wave_2_lead, CATI_imp))
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++#
  ...
  data_cohort_profile_prep_1 %>%
    ...
    select(ID_t, max1, max2) %>% distinct() %>%
  ...
  data_cohort_profile_prep_1 %>%
    select(ID_t, wave_2, treatment_starts) %>%
    na.omit() %>% pull(wave_2) %>% unique()
  
  data_cohort_profile_prep_1 %>%
    select(ID_t, wave_2, treatment_ends) %>%
    na.omit() %>% pull(wave_2) %>% unique()
...
if (cohort_prep == "controls_bef_outcome") {
  ...
  df_check_length_start <- df_check_length %>% 
    select(ID_t, interview_date, treatment_starts) %>% 
    na.omit() %>%
...
if (cohort_prep == "controls_same_outcome") {
  data_cohort_profile_save <- "Data/Prep_2/Personality/prep_2_cohort_profile_personality.rds"
} else {
  data_cohort_profile_save <- "Data/Prep_2/Personality/prep_2_cohort_profile_personality_robustcheck.rds"
}
