...
for (mice_data_sel in 1:5) {
  ...
  if (cohort_prep == "controls_same_outcome") {
    saveRDS(data_binary, paste0("Data/Grades/Prep_11/prep_11_dml_binary_all_", treatment_def, "_", 
                                treatment_repl, extra_act_save, "_mice", mice_data_sel, ".rds"))
  } else {
    saveRDS(data_binary, paste0("Data/Grades/Prep_11/prep_11_dml_binary_all_", treatment_def, "_", 
  ...
  ## 4.) Drop interactions containing 95% or more empty cells
  ## only for those with at least 95% of zeros this is operation relevant
  colnames_interaction <- colnames(df_interaction[, which(as.numeric(colSums(df_interaction == 0) / nrow(df_interaction)) >= 0.95)])
  ...
  if (cohort_prep == "controls_same_outcome") {
    saveRDS(data_all_plus, 
            paste0("Data/Grades/Prep_11/prep_11_dml_binary_allintpoly_", treatment_def, "_",
                   treatment_repl, extra_act_save, "_mice", mice_data_sel, ".rds")
            )
  } else {
    saveRDS(data_all_plus, 
  ...
  if (cohort_prep == "controls_same_outcome") {
    saveRDS(data_multi_all, 
            paste0("Data/Grades/Prep_11/prep_11_dml_multi_all_", treatment_def, "_",
                   treatment_repl, extra_act_save, "_mice", mice_data_sel, ".rds"))
  } else {
    saveRDS(data_multi_all, 
            paste0("Data/Grades/Prep_11/prep_11_dml_multi_all_", treatment_def, "_",
