...
pred_matrix_vars_num <- pred_matrix %>% as.data.frame() %>% 
  dplyr::select(starts_with("personality"), starts_with("bigfive")) %>% 
  colnames()
pred_matrix_vars_set_0 <- pred_matrix %>% as.data.frame() %>% 
  dplyr::select(!c(starts_with("personality"), starts_with("bigfive"))) %>% 
  colnames()
pred_matrix[pred_matrix_vars_num, c(pred_matrix_vars_set_0)] <- 0
...
for (mice_result_sel in 1:mice_num_data_sets) {
  ...
  if (aggr_vars == "mean") {
    ...
    list_cawi_labels <- readRDS("Data/Grades/Prep_1/prep_1_target_cawi_list.rds")
    list_cati_labels <- readRDS("Data/Grades/Prep_1/prep_1_target_cati_list.rds")
  ...
  if (cohort_prep == "controls_same_outcome") {
    ...
    data_save <- paste0("Data/Grades/Prep_7/prep_7_control_vars_", treatment_def, "_", 
                        treatment_repl, extra_act_save, "_robustcheck_", 
                        cohort_prep, "_mice", mice_result_sel, ".rds")   
  }
