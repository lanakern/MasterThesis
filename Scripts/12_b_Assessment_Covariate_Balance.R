...
# descriptive statistics for them using multivalued treatment setting
data_all_mice_grades_multi <- data.frame()
for (mice_data_sel in 1:5) {
  data_load <- paste0("Data/Grades/Prep_10/prep_10_dml_multi_all_weekly_down_extradrop",
                      cov_balance_save, "_mice", mice_data_sel, ".rds")
...
cols_multi <- colnames(data_all_mice_grades_multi_noextra)[colnames(data_all_mice_grades_multi_noextra) %in% df_main_drivers$control_var]
cols_multi <- c(cols_multi, "friends_study_share_(almost)half")
...
saveRDS(df_descr_all %>% as.data.frame(), "Output/DML/Covariate_Balancing/main_drivers_descr.rds")
