...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#
#### RC: NO EXTRACURRICULAR ACTIVITY ####
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#

df_smd_sum_multi_rc_extraact <- data.frame()
df_smd_all_multi_rc_extraact <- data.frame()
df_iterate <- data.frame("Rep" = c(rep(1,4), rep(2,4), rep(3,4), rep(4,4), rep(5,4)), "Fold" = rep(c(1:4), 5))
...
for (outcome_var_sel in c("grades")) {
  ...
  # load data 
  if (str_detect(outcome_var_sel, "grade")) {
    dml_result_all_multi_rc_extraact <- 
      readRDS(paste0("Output/DML/Estimation/Grades/multi_", outcome_var_sel, 
                     "_postlasso_all_controlssameoutcome_", treatment_def, 
  ...
  df_smd_cov_func_all_multi_rc_extraact <- data.frame()
  df_smd_cov_func_all_detail_multi_rc_extraact <- data.frame()
  for (mice_sel in 1:5) { # iterate over MICE data sets
    print(paste("Data Set:", mice_sel))
    for (rep_sel in 1:5) { # iterate over repetitions
      df_iterate_sel <- df_iterate %>% filter(Rep == rep_sel)
      for (iter_sel in 1:nrow(df_iterate_sel)) { # iterate over folds
        ...
        # predictions
        df_pred_cov_func <- dml_result_all_multi_rc_extraact[[mice_sel]]$pred %>% 
          filter(Repetition == rep_sel, Fold == iter_sel) %>%
          mutate(D_1 = ifelse(treatment == 1, 1, 0), D_2 = ifelse(treatment == 2, 1, 0), D_3 = ifelse(treatment == 3, 1, 0))
        
        # control variables
        df_controls_multi_rc_extraact <- dml_result_all_multi_rc_extraact[[mice_sel]]$cov_balance[[mice_sel]][[df_iterate_sel_2$Rep]][[df_iterate_sel_2$Fold]]$controls
        
        # control variables
        if (outcome_var_sel == "grades") {
          x <- df_controls_multi_rc_extraact %>% dplyr::select(-all_of(
            c("outcome_grade", "treatment_sport_freq", "treatment_sport_freq_na", "treatment_sport_freq_monthly_less", "treatment_sport_freq_never",
              "treatment_sport_freq_weekly_atleast", "Fold", "Repetition", "group")))
        } else {
          x <- df_controls_multi_rc_extraact %>% dplyr::select(-all_of(
        ...
        for (treatment_var_sel in c("D_1", "D_2", "D_3")) { # iterate over treatments
          ...
          # calculate weights
          weights_multi_rc_extraact <- func_weights("binary", df_pred_cov_func %>% mutate(treatment = D, m = m), x)
          
          # calculate ASMD
          balance <- bal.tab(
            as.data.frame(x), treat = D, stats = "mean.diffs", weights = weights_multi_rc_extraact, method = "weighting",
            s.d.denom = "pooled", # pooled standard deviation (most appropriate for ATE; for ATTE: "treated")
            disp.v.ratio = TRUE, disp.ks = TRUE, 
            un = TRUE, # display statistics also for before DML
            continuous = "std", binary = "std" # also standardized multi covariates
        ...
        df_smd_cov_func_all_detail_multi_rc_extraact <- rbind(df_smd_cov_func_all_detail_multi_rc_extraact, df_smd_cov_func_all)
        ...
        df_smd_cov_func_all_multi_rc_extraact <- rbind(df_smd_cov_func_all_multi_rc_extraact, df_smd_cov_func_all)
      } # close for loop over folds
    } # close iteration over repetitions
  } # close iteration over for mice data sets
  
  # summarize across controls: MICE, repetitions and folds
  df_smd_cov_func_all_multi_rc_extraact <- df_smd_cov_func_all_multi_rc_extraact %>%
    ungroup() %>%
    dplyr::select(-c("Rep", "MICE", "Fold")) %>%
    group_by(control_var) %>%
    summarize(SD_before = mean(SD_before), SD_after = mean(SD_after)) %>% 
  ...
  df_smd_cov_func_all_detail_multi_rc_extraact <- df_smd_cov_func_all_detail_multi_rc_extraact %>%
    ungroup() %>%
    dplyr::select(-c("Rep", "MICE", "Fold")) %>%
    group_by(control_var) %>%
    summarize(SD_before_D_1 = mean(SD_before_D_1), SD_before_D_2 = mean(SD_before_D_2), 
  ...
  # summary statistics
  df_smd_cov_func_sum_multi_rc_extraact <- 
    data.frame(
      "treatment_setting" = rep("multi", 2),
      "adjustment" = c("before", "after"),
      "min" = c(min(df_smd_cov_func_all_multi_rc_extraact$SD_before),  min(df_smd_cov_func_all_multi_rc_extraact$SD_after, na.rm = T)),
      "mean" = c(mean(df_smd_cov_func_all_multi_rc_extraact$SD_before), mean(df_smd_cov_func_all_multi_rc_extraact$SD_after, na.rm = T)),
      "median" = c(median(df_smd_cov_func_all_multi_rc_extraact$SD_before), median(df_smd_cov_func_all_multi_rc_extraact$SD_after, na.rm = T)),
      "max" = c(max(df_smd_cov_func_all_multi_rc_extraact$SD_before), max(df_smd_cov_func_all_multi_rc_extraact$SD_after, na.rm = T)),
      "num_cov_smd_25" = c(sum(df_smd_cov_func_all_multi_rc_extraact$SD_before > 0.25), 
                           sum(df_smd_cov_func_all_multi_rc_extraact$SD_after > 0.25, na.rm = T)), 
      "num_cov_smd_10" = c(sum(df_smd_cov_func_all_multi_rc_extraact$SD_before > 0.1), 
                           sum(df_smd_cov_func_all_multi_rc_extraact$SD_after > 0.1, na.rm = T)), 
      "num_cov_smd_5" = c(sum(df_smd_cov_func_all_multi_rc_extraact$SD_before > 0.05), 
                          sum(df_smd_cov_func_all_multi_rc_extraact$SD_after > 0.05, na.rm = T)), 
      "perc_cov_smd_25" = c(sum(df_smd_cov_func_all_multi_rc_extraact$SD_before > 0.25) / length(df_smd_cov_func_all_multi_rc_extraact$SD_before), 
                            sum(df_smd_cov_func_all_multi_rc_extraact$SD_after > 0.25, na.rm = T) / df_smd_cov_func_all_multi_rc_extraact %>% dplyr::select(SD_after) %>% na.omit() %>% nrow()), 
      "perc_cov_smd_10" = c(sum(df_smd_cov_func_all_multi_rc_extraact$SD_before > 0.1) / length(df_smd_cov_func_all_multi_rc_extraact$SD_before), 
                            sum(df_smd_cov_func_all_multi_rc_extraact$SD_after > 0.1, na.rm = T) / df_smd_cov_func_all_multi_rc_extraact %>% dplyr::select(SD_after) %>% na.omit() %>% nrow()), 
      "perc_cov_smd_5" = c(sum(df_smd_cov_func_all_multi_rc_extraact$SD_before > 0.05) / length(df_smd_cov_func_all_multi_rc_extraact$SD_before), 
                           sum(df_smd_cov_func_all_multi_rc_extraact$SD_after > 0.05, na.rm = T) / df_smd_cov_func_all_multi_rc_extraact %>% dplyr::select(SD_after) %>% na.omit() %>% nrow())
    )
  
  df_smd_cov_func_sum_multi_rc_extraact <- df_smd_cov_func_sum_multi_rc_extraact %>% mutate(outcome = outcome_var_sel)
  df_smd_cov_func_all_multi_rc_extraact <- df_smd_cov_func_all_multi_rc_extraact %>% mutate(outcome = outcome_var_sel)
  
  # append to overall data frame
  df_smd_sum_multi_rc_extraact <- rbind(df_smd_sum_multi_rc_extraact, df_smd_cov_func_sum_multi_rc_extraact)
  df_smd_all_multi_rc_extraact <- rbind(df_smd_all_multi_rc_extraact, df_smd_cov_func_all_multi_rc_extraact)
  
} # close loop over outcome_var_sel()

df_smd_sum_multi_rc_extraact <- df_smd_sum_multi_rc_extraact %>% dplyr::select(
  outcome, treatment_setting, adjustment, everything()
)

df_smd_all_multi_rc_extraact <- df_smd_all_multi_rc_extraact %>% dplyr::select(
  outcome, control_var, SD_before, SD_after
)

# aggregate over personality
df_smd_sum_multi_rc_extraact <- rbind(
  df_smd_sum_multi_rc_extraact,
  df_smd_sum_multi_rc_extraact %>% 
    filter(outcome != "grades") %>%
    mutate(outcome = "personality") %>%
    group_by(outcome, treatment_setting, adjustment) %>%
...
saveRDS(df_smd_sum_multi_rc_extraact, "Output/DML/Covariate_Balancing/covariate_balancing_summary_multi_noextra.rds")


#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#
#### RC: NO ENDOGENOUS VARIABLES ####
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#

df_smd_sum_multi_rc_noendog <- data.frame()
df_smd_all_multi_rc_noendog <- data.frame()
df_iterate <- data.frame("Rep" = c(rep(1,4), rep(2,4), rep(3,4), rep(4,4), rep(5,4)), "Fold" = rep(c(1:4), 5))
...
for (outcome_var_sel in c("grades")) {
  ...
  # load data 
  dml_result_all_multi_rc_noendog <- 
    readRDS(paste0("Output/DML/Estimation/Grades/multi_", outcome_var_sel, 
                   "_postlasso_all_controlssameoutcome_", treatment_def, 
                   "_down_extradrop_all_notreatmentoutcomelags_endogno_trimming",
                   model_trimming, "_K4-2_Rep5", cov_balance_save, ".rds"))
  
  df_smd_cov_func_all_multi_rc_noendog <- data.frame()
  df_smd_cov_func_all_detail_multi_rc_noendog <- data.frame()
  for (mice_sel in 1:5) { # iterate over MICE data sets
    print(paste("Data Set:", mice_sel))
    for (rep_sel in 1:5) { # iterate over repetitions
      df_iterate_sel <- df_iterate %>% filter(Rep == rep_sel)
      for (iter_sel in 1:nrow(df_iterate_sel)) { # iterate over folds
        ...
        # predictions
        df_pred_cov_func <- dml_result_all_multi_rc_noendog[[mice_sel]]$pred %>% 
          filter(Repetition == rep_sel, Fold == iter_sel) %>%
          mutate(D_1 = ifelse(treatment == 1, 1, 0), D_2 = ifelse(treatment == 2, 1, 0), D_3 = ifelse(treatment == 3, 1, 0))
        
        # control variables
        df_controls_multi_rc_noendog <- dml_result_all_multi_rc_noendog[[mice_sel]]$cov_balance[[mice_sel]][[df_iterate_sel_2$Rep]][[df_iterate_sel_2$Fold]]$controls
        
        # control variables
        if (outcome_var_sel == "grades") {
          x <- df_controls_multi_rc_noendog %>% dplyr::select(-all_of(
            c("outcome_grade", "treatment_sport_freq", "treatment_sport_freq_na", "treatment_sport_freq_monthly_less", "treatment_sport_freq_never",
              "treatment_sport_freq_weekly_atleast", "Fold", "Repetition", "group")))
        } else {
          x <- df_controls_multi_rc_noendog %>% dplyr::select(-all_of(
        ...
        for (treatment_var_sel in c("D_1", "D_2", "D_3")) { # iterate over treatments
          ...
          # calculate weights
          weights_multi_rc_noendog <- func_weights("binary", df_pred_cov_func %>% mutate(treatment = D, m = m), x)
          
          # calculate ASMD
          balance <- bal.tab(
            as.data.frame(x), treat = D, stats = "mean.diffs", weights = weights_multi_rc_noendog, method = "weighting",
            s.d.denom = "pooled", # pooled standard deviation (most appropriate for ATE; for ATTE: "treated")
            disp.v.ratio = TRUE, disp.ks = TRUE, 
            un = TRUE, # display statistics also for before DML
            continuous = "std", binary = "std" # also standardized multi covariates
        ...
        df_smd_cov_func_all_detail_multi_rc_noendog <- rbind(df_smd_cov_func_all_detail_multi_rc_noendog, df_smd_cov_func_all)
        ...
        df_smd_cov_func_all_multi_rc_noendog <- rbind(df_smd_cov_func_all_multi_rc_noendog, df_smd_cov_func_all)
      } # close for loop over folds
    } # close iteration over repetitions
  } # close iteration over for mice data sets
  
  # summarize across controls: MICE, repetitions and folds
  df_smd_cov_func_all_multi_rc_noendog <- df_smd_cov_func_all_multi_rc_noendog %>%
    ungroup() %>%
    dplyr::select(-c("Rep", "MICE", "Fold")) %>%
    group_by(control_var) %>%
    summarize(SD_before = mean(SD_before), SD_after = mean(SD_after)) %>% 
  ...
  df_smd_cov_func_all_detail_multi_rc_noendog <- df_smd_cov_func_all_detail_multi_rc_noendog %>%
    ungroup() %>%
    dplyr::select(-c("Rep", "MICE", "Fold")) %>%
    group_by(control_var) %>%
    summarize(SD_before_D_1 = mean(SD_before_D_1), SD_before_D_2 = mean(SD_before_D_2), 
  ...
  # summary statistics
  df_smd_cov_func_sum_multi_rc_noendog <- 
    data.frame(
      "treatment_setting" = rep("multi", 2),
      "adjustment" = c("before", "after"),
      "min" = c(min(df_smd_cov_func_all_multi_rc_noendog$SD_before),  min(df_smd_cov_func_all_multi_rc_noendog$SD_after, na.rm = T)),
      "mean" = c(mean(df_smd_cov_func_all_multi_rc_noendog$SD_before), mean(df_smd_cov_func_all_multi_rc_noendog$SD_after, na.rm = T)),
      "median" = c(median(df_smd_cov_func_all_multi_rc_noendog$SD_before), median(df_smd_cov_func_all_multi_rc_noendog$SD_after, na.rm = T)),
      "max" = c(max(df_smd_cov_func_all_multi_rc_noendog$SD_before), max(df_smd_cov_func_all_multi_rc_noendog$SD_after, na.rm = T)),
      "num_cov_smd_25" = c(sum(df_smd_cov_func_all_multi_rc_noendog$SD_before > 0.25), 
                           sum(df_smd_cov_func_all_multi_rc_noendog$SD_after > 0.25, na.rm = T)), 
      "num_cov_smd_10" = c(sum(df_smd_cov_func_all_multi_rc_noendog$SD_before > 0.1), 
                           sum(df_smd_cov_func_all_multi_rc_noendog$SD_after > 0.1, na.rm = T)), 
      "num_cov_smd_5" = c(sum(df_smd_cov_func_all_multi_rc_noendog$SD_before > 0.05), 
                          sum(df_smd_cov_func_all_multi_rc_noendog$SD_after > 0.05, na.rm = T)), 
      "perc_cov_smd_25" = c(sum(df_smd_cov_func_all_multi_rc_noendog$SD_before > 0.25) / length(df_smd_cov_func_all_multi_rc_noendog$SD_before), 
                            sum(df_smd_cov_func_all_multi_rc_noendog$SD_after > 0.25, na.rm = T) / df_smd_cov_func_all_multi_rc_noendog %>% dplyr::select(SD_after) %>% na.omit() %>% nrow()), 
      "perc_cov_smd_10" = c(sum(df_smd_cov_func_all_multi_rc_noendog$SD_before > 0.1) / length(df_smd_cov_func_all_multi_rc_noendog$SD_before), 
                            sum(df_smd_cov_func_all_multi_rc_noendog$SD_after > 0.1, na.rm = T) / df_smd_cov_func_all_multi_rc_noendog %>% dplyr::select(SD_after) %>% na.omit() %>% nrow()), 
      "perc_cov_smd_5" = c(sum(df_smd_cov_func_all_multi_rc_noendog$SD_before > 0.05) / length(df_smd_cov_func_all_multi_rc_noendog$SD_before), 
                           sum(df_smd_cov_func_all_multi_rc_noendog$SD_after > 0.05, na.rm = T) / df_smd_cov_func_all_multi_rc_noendog %>% dplyr::select(SD_after) %>% na.omit() %>% nrow())
    )
  
  df_smd_cov_func_sum_multi_rc_noendog <- df_smd_cov_func_sum_multi_rc_noendog %>% mutate(outcome = outcome_var_sel)
  df_smd_cov_func_all_multi_rc_noendog <- df_smd_cov_func_all_multi_rc_noendog %>% mutate(outcome = outcome_var_sel)
  
  # append to overall data frame
  df_smd_sum_multi_rc_noendog <- rbind(df_smd_sum_multi_rc_noendog, df_smd_cov_func_sum_multi_rc_noendog)
  df_smd_all_multi_rc_noendog <- rbind(df_smd_all_multi_rc_noendog, df_smd_cov_func_all_multi_rc_noendog)
  
} # close loop over outcome_var_sel()

df_smd_sum_multi_rc_noendog <- df_smd_sum_multi_rc_noendog %>% dplyr::select(
  outcome, treatment_setting, adjustment, everything()
)

df_smd_all_multi_rc_noendog <- df_smd_all_multi_rc_noendog %>% dplyr::select(
  outcome, control_var, SD_before, SD_after
)

# aggregate over personality
df_smd_sum_multi_rc_noendog <- rbind(
  df_smd_sum_multi_rc_noendog,
  df_smd_sum_multi_rc_noendog %>% 
    filter(outcome != "grades") %>%
    mutate(outcome = "personality") %>%
    group_by(outcome, treatment_setting, adjustment) %>%
...
saveRDS(df_smd_sum_multi_rc_noendog, "Output/DML/Covariate_Balancing/covariate_balancing_summary_multi_noendog.rds")
    ...
    for (rep_sel in 1:5) { # iterate over repetitions
      ...
      for (iter_sel in 1:nrow(df_iterate_sel)) { # iterate over folds
        ...
        df_smd_cov_func_all <- df_smd_cov_func_all %>%
          ...
          group_by(control_var, Rep, Fold, MICE) %>%
          summarize(SD_before = max(c(SD_before_D_1, SD_before_D_2, SD_before_D_3)),
                    SD_after = mmax(c(SD_after_D_1, SD_after_D_2, SD_after_D_3))) %>%
          ungroup()
